"""
The Void -- Webhook Receiver Layer
...
Receives inbound webhooks from:
  ...
  - Generic deployment hooks (any ZQM repo deployment)
  - Docker Registry (image push events)

All webhooks are authenticated and fed into ZQM_AI's edge connector
...
if not ZQM_INTERNAL_KEY:
    logger.warning("ZQM_INTERNAL_KEY env not set — webhook signature verification disabled")
...
def _verify_azure_devops_signature(body: bytes, signature: str, secret: str) -> bool:
    ...
    Azure DevOps webhooks are protected by an HTTP basic-auth secret. If no
    secret is configured, verification fails closed (reject unauthenticated
    webhooks) rather than silently accepting them.
    """
    if not secret:
        return False
    # `signature` here carries the expected shared secret from the request;
...
async def _ingest_webhook_event(
    ...
    """
    Feed a webhook event into The Void:
      1. publish to the in-process event bus (SSE consumers, logs)
      2. audit to FLATSPACE waxcell (immutable)
      3. create an orchestrator task so The Void can ACT on it
    ...
    # 1) event bus (real-time SSE)
    try:
        await bus.publish("webhook", payload)
    except Exception as exc:
        logger.warning("[webhook] bus publish failed: %s", exc)

    # 2) FLATSPACE audit (immutable)
    orch = _get_orchestrator()
    if orch is not None:
        try:
            await orch.flatspace.store(
                key=f"webhook:{source}:{event_type}:{data.get('repository', data.get('app_name', 'x'))}",
                value=payload,
                tier="waxcell",
            )
        except Exception as exc:
    ...
    return {"status": "ingested", "source": source, "event_type": event_type}


# Module-level orchestrator reference (set at app startup via set_orchestrator).
ORCHESTRATOR = None
...
async def github_webhook(
    ...
    Configure in GitHub repo settings → Webhooks → Add webhook:
      Payload URL: http://<zqm_ai-host>:8808/api/webhook/github
...
async def azure_devops_webhook(request: Request):
    ...
    Configure in Azure DevOps → Project Settings → Service Hooks:
      ...
      URL: http://<zqm_ai-host>:8808/api/webhook/azure-devops
    """
    body = await request.body()
    payload = json.loads(body)
