"""
The Void AI Orchestration System — VoidCache
...
Architecture note: VoidCache is the "AI volatile" tier (Level 6)
in the ZQM FLATSPACE tiered memory stack. For persistent storage, use
FlatSpaceService which routes to PollenStore (cold) or BitGarden (hot).
"""
...
class VoidCache:
    ...
    Designed for The Void's short-term cognitive memory:
    - Stores intermediate task results, agent responses, context windows
    - Max 512 slots by default (configurable via VOID_CACHE_MAX_SIZE)
    ...
    async def purge_expired(self) -> int:
        ...
        async with self._lock:
            ...
            for key in expired:
                ...
                self._expirations += 1
        if expired:
            log.debug("VoidCache purged expired entries", count=len(expired))
        return len(expired)
...
def get_void_cache() -> VoidCache:
    """Return the global VoidCache singleton (lazy-initialized)."""
    global _cache_instance
    if _cache_instance is None:
        _cache_instance = VoidCache(
            max_size=settings.void_cache_max_size,
            strategy=settings.void_cache_strategy,
