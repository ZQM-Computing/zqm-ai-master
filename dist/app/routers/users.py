"""
The Void AI Orchestration System — /api/users Router
...
async def login(credentials: UserLogin) -> ZQM_AIResponse:
    """
    Authenticate a user and return a JWT access token.

    **Default admin credentials (change in production!):**
    - username: `admin`
    - password: `zqm_ai-admin` (override via ZQM_ADMIN_PASSWORD env)
    """
...
async def list_users(
    ...
    return ZQM_AIResponse.ok(data=users_out, message=f"{len(users_out)} user(s)")
...
async def generate_user_api_key(
    ...
    # Store a bcrypt HASH, never the plaintext (the plaintext is returned once
    # to the caller and should be persisted by the client, like a password).
    user.api_key_hash = hash_password(api_key)  # type: ignore[attr-defined]
