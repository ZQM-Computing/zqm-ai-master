"""
The Void AI Orchestration System — /api/predict Router
...
async def list_models() -> ZQM_AIResponse:
    ...
    return ZQM_AIResponse.ok(data=models, message="Available models")
...
async def _call_provider(provider: str, model: str, request_body: PredictRequest) -> Any:
    ...
    if provider in ("openai", "anthropic") and not settings.allow_external_providers:
        raise HTTPException(
            ...
            detail="External AI provider blocked: The Void is self-hosted. "
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        if provider == "ollama":
            ...
            return {
                ...
                "federated": data.get("_federated", False),
            }

        elif provider == "local_deterministic":
            from app.orchestrator.cognitive_processor import CognitiveProcessor
            cp = CognitiveProcessor()
            output = await cp._call_local_deterministic(
                type("Agent", (), {"name": model, "system_prompt": None, "capabilities": []})(),
                messages,
                {"max_tokens": params.get("max_tokens", 2048), "temperature": params.get("temperature", 0.7)},
            )
            return {"content": output, "backend": "local_deterministic", "federated": False}

        elif provider == "embedding":
            from app.services.mesh_ollama import router as mesh_ollama
            emb_model = model or "nomic-embed-text:latest"
            data = await mesh_ollama.embed(
                text=request_body.input, model=emb_model, timeout=60.0,
            )
